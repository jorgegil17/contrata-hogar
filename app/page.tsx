...
function Dashboard() {
  ...
  const [weeklyHoursInput, setWeeklyHoursInput] = useState(String(defaults.weeklyHours));
  const [hourlyRateInput, setHourlyRateInput] = useState(String(defaults.hourlyRate).replace('.', ','));
  ...
  const contractNotStarted = contractStartsLater;
  ...
  // Las horas adicionales se introducen como tiempo y se valoran con el salario
  ...
  const contributions = contractNotStarted
    ? { ...estimatedContributions, base: 0, worker: 0, employer: 0, directDebit: 0, net: 0, totalCost: 0 }
    : { ...estimatedContributions, worker: workerContribution, employer: employerContribution, directDebit: cents(workerContribution + employerContribution), net: cents(monthlyGross - workerContribution), totalCost: cents(monthlyGross + employerContribution) };
  ...
  const partiesComplete = Boolean(contract.employerName && contract.employerDni && contract.employerAddress && contract.employerMunicipality && contract.employerPostcode && contract.contributionAccount && contract.employeeName && contract.employeeDni && contract.employeeNss && contract.employeeAddress && contract.employeeNationality && contract.employeeMunicipality && contract.employeePostcode);
  ...
  const openContractWizard = () => { setDraft(contract); setWeeklyHoursInput(String(contract.weeklyHours).replace('.', ',')); setHourlyRateInput(String(contract.hourlyRate).replace('.', ',')); setWizardStep(1); setContractWizard(true); };
  const saveWizard = () => {
    if (!draft.startDate || draft.weeklyHours <= 0 || draft.hourlyRate <= 0 || !draft.workAddress.trim() || !draft.scheduleEntries.length || !draftHoursMatch || !draft.employerName || !draft.employeeName || !draft.employeeNss) { notify(draftHoursMatch ? 'Completa los datos necesarios para calcular la nómina' : 'El horario debe sumar exactamente las horas semanales pactadas'); return; }
    setContract(draft);
  ...
  return <main className="shell">
    ...
    <section className="workspace" id="top">
      <header><div><small>{new Intl.DateTimeFormat('es-ES', { weekday: 'long', day: 'numeric', month: 'long' }).format(new Date()).toUpperCase()}</small><h1>{activeView === 'home' ? 'Buenos días, Jorge' : activeView === 'contract' ? 'Condiciones' : activeView === 'attendance' ? 'Asistencia' : activeView === 'annual' ? 'Resumen anual' : 'Documentos'}</h1></div><button className="help" onClick={() => notify('Centro de ayuda: disponible en la siguiente versión')}>ⓘ Ayuda</button></header>
      <div className="content">
        ...
        <Title title="Este mes" action="Abrir asistencia →" onClick={() => setActiveView('attendance')} /><p className="estimate-note">Importes estimados. La cotización válida es la comunicada por la TGSS; al introducirla en asistencia aparecerá como importe real.</p>
        <div className="month month-home">
          <article className={'pay ' + (payrollPrepared ? 'done' : '')}>{contractNotStarted ? <><div className="cardtop"><b>i</b><span>CONTRATO PENDIENTE DE INICIO</span></div><p>El contrato comienza en {monthLabel(contract.startDate.slice(0, 7))}</p><div className="prestart-note">Todavía no hay nómina ni cotizaciones que calcular. La asistencia estará disponible al comenzar el contrato.</div></> : <><div className="cardtop"><b>€</b><span>{payrollPrepared ? 'RECIBO DISPONIBLE' : 'CIERRA LA ASISTENCIA'}</span></div><p>Nómina de {monthLabel(attendanceMonth)}</p><div className="monthly-amounts"><div><span>Neto a transferir</span><strong>{euro(net)}</strong></div><div><span>Aportación de la persona trabajadora</span><strong>{euro(contributions.worker)}</strong></div><div><span>Aportación de la persona empleadora</span><strong>{euro(contributions.employer)}</strong></div></div><div className="actions"><button className="primary" disabled={!payrollPrepared} onClick={generateReceipt}>Descargar recibo PDF</button><button onClick={() => setActiveView('attendance')}>Ver asistencia</button></div></>}</article>
        </div>
        </>}

        {activeView === 'contract' && <section className="app-view contract-view"><div className="view-head"><div><span>RELACIÓN LABORAL</span><h2>Parámetros del contrato</h2><p>Consulta y actualiza aquí los datos del acuerdo que utilizan la asistencia, las nóminas y los resúmenes. El contrato firmado se prepara fuera de la aplicación.</p></div></div><section className="contract-card" id="contrato"><div className="contract-icon">✓</div><div className="contract-copy"><span>{contractReady ? 'PARÁMETROS COMPLETOS' : 'DATOS PENDIENTES'}</span><h2>Ficha de condiciones</h2><p>{contract.weeklyHours.toLocaleString('es-ES')} horas semanales · {euro(contract.hourlyRate)}/hora · {euro(salary)} brutos/mes estimados</p><div className="contract-checks"><span>{partiesComplete ? '✓ Datos administrativos completos' : '○ Faltan datos administrativos'}</span><span>{Math.abs(contractScheduleHours - contract.weeklyHours) < .01 ? '✓ Horario y jornada coinciden' : `○ El horario suma ${contractScheduleHours.toLocaleString('es-ES')} h`}</span></div></div><div className="contract-actions"><button className="primary" onClick={openContractWizard}>Editar parámetros</button><button onClick={generateManagementSheet}>Ficha interna PDF</button></div></section><article className="person" id="persona"><div className="personhead"><b>{contract.employeeName ? contract.employeeName.slice(0, 2).toUpperCase() : 'TR'}</b><p><strong>{contract.employeeName || 'Persona trabajadora'}</strong><small>Ficha informativa · Contrato indefinido activo</small></p><span className="source-badge">Fuente única de datos</span></div><dl><div><dt>Antigüedad</dt><dd>{seniority(contract.startDate)}<small>Inicio: {formatDate(contract.startDate)}</small></dd></div><div><dt>Jornada</dt><dd>{contract.weeklyHours.toLocaleString('es-ES')} h / semana<small>Horario acordado</small></dd></div><div><dt>Salario por hora</dt><dd>{euro(contract.hourlyRate)}<small>{euro(salary)} brutos / mes estimados</small></dd></div></dl></article></section>}

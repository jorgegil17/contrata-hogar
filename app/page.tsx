...
export default function Home() {
  ...
  return <main className="shell">
    ...
    <section className="workspace" id="top">
      <header><div><small>{new Intl.DateTimeFormat('es-ES', { weekday: 'long', day: 'numeric', month: 'long' }).format(new Date()).toUpperCase()}</small><h1>{activeView === 'home' ? 'Buenos días, Jorge' : activeView === 'contract' ? 'Contrato' : activeView === 'attendance' ? 'Asistencia' : activeView === 'annual' ? 'Resumen anual' : 'Documentos'}</h1></div><button className="help" onClick={() => notify('Centro de ayuda: disponible en la siguiente versión')}>ⓘ Ayuda</button></header>

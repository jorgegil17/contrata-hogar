...
export default function Home() {
  ...
  const generateReceipt = () => {
    ...
    pdf.setFont('helvetica', 'bold').text('A. TOTAL DEVENGADO', left + 3, 140).text(euro(salary), 190, 140, { align: 'right' });
    ...
    pdf.setFont('helvetica', 'normal').setFontSize(7).text('Aportación de la persona trabajadora a la Seguridad Social (estimada)', left + 3, 169).text(euro(deduction), 190, 169, { align: 'right' });
    pdf.text('Retención IRPF', left + 3, 177).text('No calculada', 190, 177, { align: 'right' });
  ...
  const generateContract = (finalDocument = false) => {
    ...
    const width = 170;
    let y = 18;
    const pageHeader = (page: number) => { if (page > 1) { pdf.addPage(); y = 18; } pdf.setTextColor(8, 125, 189).setFont('helvetica', 'bold').setFontSize(8).text('CONTRATA HOGAR · ANEXO AL CONTRATO INDEFINIDO', left, y); pdf.setFont('helvetica', 'normal').setTextColor(120, 137, 145).text(`Página ${page} de 2`, left + width, y, { align: 'right' }); y += 9; };
    const heading = (title: string) => { pdf.setFont('helvetica', 'bold').setFontSize(10.3).setTextColor(23, 36, 43).text(title, left, y); y += 2.3; pdf.setDrawColor(204, 217, 223).line(left, y, left + width, y); y += 5.2; };
    const paragraph = (text: string) => { pdf.setFont('helvetica', 'normal').setFontSize(7.8).setTextColor(45, 59, 66); const lines = pdf.splitTextToSize(text, width); pdf.text(lines, left, y); y += lines.length * 3.55 + 2.2; };
    pageHeader(1);
    pdf.setTextColor(23, 36, 43).setFont('helvetica', 'bold').setFontSize(20).text('Anexo de condiciones particulares', left, y);
    y += 8; pdf.setTextColor(8, 125, 189).setFontSize(15).text('Contrato indefinido a tiempo parcial', left, y);
    y += 7; pdf.setFont('helvetica', 'normal').setFontSize(10).setTextColor(95, 117, 128).text('Servicio del hogar familiar · Para firmar junto al modelo oficial del SEPE', left, y); y += 13;
    ...
    y += 40;
    paragraph('Ambas partes incorporan este anexo al modelo oficial de contrato de trabajo indefinido del SEPE, al objeto de detallar de forma clara las condiciones particulares ya acordadas para el servicio del hogar familiar.');
    ...
    if (!finalDocument) { pdf.setFillColor(255, 247, 231).rect(left, 276, width, 8, 'F'); pdf.setFont('helvetica', 'bold').setTextColor(110, 91, 45).text('BORRADOR DEL ANEXO - Revisar antes de firmar.', left + 4, 281); }
    pdf.save(`${finalDocument ? 'anexo-definitivo' : 'borrador-anexo'}-${contract.startDate}.pdf`);
    notify(finalDocument ? 'Anexo definitivo descargado en PDF' : 'Borrador del anexo descargado en PDF');
  };
  const confirmFinalContract = () => {
    if (window.confirm('¿Confirmas que has revisado todos los datos y quieres generar el anexo definitivo para firmarlo junto al modelo oficial del SEPE?')) generateContract(true);
  };
  ...
  return <main className="shell">
    ...
    <section className="workspace" id="top">
      ...
      <div className="content">
        ...
        {activeView === 'contract' && <section className="app-view contract-view"><div className="view-head"><div><span>RELACIÓN LABORAL</span><h2>Paquete contractual</h2><p>Firma conjuntamente el modelo oficial del SEPE y el anexo de condiciones generado por Contrata Hogar.</p></div></div><section className="official-contract"><div className="official-step"><b>1</b><div><span>DOCUMENTO OFICIAL</span><h3>Modelo de contrato indefinido del SEPE</h3><p>Descarga el modelo oficial vigente y completa sus datos administrativos. Para este caso corresponde tiempo parcial y las cláusulas específicas del servicio del hogar familiar.</p></div><a href="https://www.sepe.es/NuevoSepe/contratos-trabajo/indefinido/Contrato-indefinido.pdf" target="_blank" rel="noreferrer">Abrir modelo oficial ↗</a></div><div className="official-note">El modelo oficial y el anexo deben conservarse y firmarse juntos. Contrata Hogar no envía ni registra el contrato ante la Administración.</div></section><section className="contract-card" id="contrato"><div className="contract-icon">2</div><div className="contract-copy"><span>{contractReady ? 'ANEXO LISTO PARA REVISAR' : 'ANEXO INCOMPLETO'}</span><h2>Anexo de condiciones particulares</h2><p>{contract.weeklyHours.toLocaleString('es-ES')} horas semanales · {euro(contract.hourlyRate)}/hora · {euro(salary)} brutos/mes</p><div className="contract-checks"><span>{partiesComplete ? '✓ Datos de ambas partes completos' : '○ Faltan datos obligatorios'}</span><span>{Math.abs(contractScheduleHours - contract.weeklyHours) < .01 ? '✓ Horario y jornada coinciden' : `○ El horario suma ${contractScheduleHours.toLocaleString('es-ES')} h`}</span></div></div><div className="contract-actions"><button onClick={openContractWizard}>Completar datos</button><button disabled={!contractReady} title={contractReady ? 'Descargar borrador del anexo' : 'Completa los datos y corrige el horario'} onClick={() => generateContract(false)}>Generar borrador del anexo</button><button className="primary" disabled={!contractReady} title={contractReady ? 'Generar anexo definitivo' : 'Completa los datos y corrige el horario'} onClick={confirmFinalContract}>Confirmar anexo definitivo</button><button onClick={generateManagementSheet}>Ficha interna PDF</button></div></section><article className="person" id="persona"><div className="personhead"><b>{contract.employeeName ? contract.employeeName.slice(0, 2).toUpperCase() : 'TR'}</b><p><strong>{contract.employeeName || 'Persona trabajadora'}</strong><small>Ficha informativa · Contrato indefinido activo</small></p><span className="source-badge">Datos del contrato</span></div><dl><div><dt>Antigüedad</dt><dd>{seniority(contract.startDate)}<small>Inicio: {formatDate(contract.startDate)}</small></dd></div><div><dt>Jornada</dt><dd>{contract.weeklyHours.toLocaleString('es-ES')} h / semana<small>Horario acordado</small></dd></div><div><dt>Salario por hora</dt><dd>{euro(contract.hourlyRate)}<small>{euro(salary)} brutos / mes estimados</small></dd></div></dl></article></section>}
        ...
        {activeView === 'documents' && <section className="app-view"><div className="view-head"><div><span>ARCHIVO</span><h2>Documentos</h2><p>Modelo oficial, anexo, recibos salariales y documentos internos.</p></div></div><div className="document-grid"><article><b>SEPE</b><h3>Modelo oficial indefinido</h3><p>Documento oficial para completar y firmar.</p><a className="document-link" href="https://www.sepe.es/NuevoSepe/contratos-trabajo/indefinido/Contrato-indefinido.pdf" target="_blank" rel="noreferrer">Abrir modelo ↗</a></article><article><b>PDF</b><h3>Borrador del anexo</h3><p>Condiciones particulares para revisar.</p><button disabled={!contractReady} onClick={() => generateContract(false)}>Descargar</button></article><article><b>PDF</b><h3>Recibo de {monthLabel(attendanceMonth)}</h3><p>{payrollPrepared ? 'Disponible tras cerrar la asistencia' : 'Cierra primero la asistencia del mes'}</p><button disabled={!payrollPrepared} onClick={generateReceipt}>Descargar</button></article><article><b>PDF</b><h3>Ficha interna</h3><p>Estimaciones y registros de gestión.</p><button onClick={generateManagementSheet}>Descargar</button></article></div></section>}
      </div>
    </section>

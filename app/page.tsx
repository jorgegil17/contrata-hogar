...
export default function Home() {
  const [activeView, setActiveView] = useState<'home' | 'attendance' | 'annual' | 'documents'>('home');
  ...
  const setPayrollPrepared = (ready: boolean) => setClosedMonths({ ...closedMonths, [attendanceMonth]: ready });
  ...
  return <main className="shell">
    ...
    <section className="workspace" id="top">
      <header><div><small>DOMINGO, 30 DE AGOSTO</small><h1>{activeView === 'home' ? 'Buenos días, Jorge' : activeView === 'attendance' ? 'Asistencia' : activeView === 'annual' ? 'Resumen anual' : 'Documentos'}</h1></div><button className="help" onClick={() => notify('Centro de ayuda: disponible en la siguiente versión')}>ⓘ Ayuda</button></header>
      <div className="content">
        ...
        <div className="month">
          <article className={'pay ' + (payrollPrepared ? 'done' : '')}><div className="cardtop"><b>€</b><span>{payrollPrepared ? 'RECIBO DISPONIBLE' : 'CIERRA LA ASISTENCIA'}</span></div><p>Nómina de {monthLabel(attendanceMonth)}</p><strong>{euro(net)}</strong><small>Neto estimado a transferir</small><div className="actions"><button className="primary" disabled={!payrollPrepared} onClick={generateReceipt}>Descargar recibo PDF</button><button onClick={() => setActiveView('attendance')}>Ver asistencia</button></div></article>

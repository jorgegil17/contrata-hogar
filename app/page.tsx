...
export default function Home() {
  ...
  const generateContract = (finalDocument = false) => {
    ...
    if (!finalDocument) { pdf.setFillColor(255, 247, 231).rect(left, 276, width, 8, 'F'); pdf.setFont('helvetica', 'bold').setTextColor(110, 91, 45).text('BORRADOR - Revisar antes de firmar o presentar.', left + 4, 281); }
    pdf.save(`${finalDocument ? 'contrato-definitivo' : 'borrador-contrato'}-${contract.startDate}.pdf`);
    notify(finalDocument ? 'Contrato definitivo descargado en PDF' : 'Borrador descargado en PDF');
  };
  const confirmFinalContract = () => {
    if (window.confirm('¿Confirmas que has revisado todos los datos y quieres generar el contrato definitivo listo para firmar?')) generateContract(true);
  };
